...
export const ALLOWED_EXTENSIONS = ["toml", "yaml", "json","yml"] as const;
...
export function tryFileExtension(path: string) {
	...
	return result !== undefined && ALLOWED_EXTENSIONS.includes(result as never);
}
export function getFileExtension(path: string) {
	if (!tryFileExtension(path))
		throw new Error(
			"BLACKLISTED FILE EXTENSION AT " +
				path +
				". PROGRAM ONLY SUPPORTS " +
				ALLOWED_EXTENSIONS.join(",") +
				" FILES",
		);
	return FILE_EXTENSION_REGEXP.exec(path)?.[1] as (typeof ALLOWED_EXTENSIONS)[number];
}
